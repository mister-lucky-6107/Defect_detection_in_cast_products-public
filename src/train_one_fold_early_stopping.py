...
def train_one_fold_early_stopping(all_images, all_labels, all_seg_masks, all_image_paths, notebook_number, n_splits=5, num_epochs=10, batch_size=16, lr=1e-4, weight=2.0, desired_fold=1, segmentation_class_weights={0: 51, 1: 85, 2: 55, 3: 218, 4: 1, 5: 200, 6: 18}, patience=5):
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(all_images, all_labels)):
        ...
        # 学習データと検証データの作成
        train_images_fold, val_images_fold = all_images[torch.tensor(train_index)], all_images[torch.tensor(val_index)]         # インデックスを利用して、画像データをこのフォールドにおける、学習用と検証用に分割
        train_labels_fold, val_labels_fold = all_labels[torch.tensor(train_index)], all_labels[torch.tensor(val_index)]         # インデックスを利用して、ラベルデータをこのフォールドにおける、学習用と検証用に分割
        train_seg_masks_fold, val_seg_masks_fold = all_seg_masks[torch.tensor(train_index)], all_seg_masks[torch.tensor(val_index)]
        train_paths_fold, val_paths_fold = all_image_paths[torch.tensor(train_index)], all_image_paths[torch.tensor(val_index)] # インデックスを利用して、画像パスをこのフォールドにおける、学習用と検証用に分割
        ...
        # フォールドごとに、オプティマイザの初期化　オプティマイザは最適化アルゴリズムを使って、損失が最小になるようにパラメータを更新
        ...
        for epoch in tqdm(range(num_epochs), desc=f"Fold {fold+1} Epochs"):
            ...
            # エポックごとに評価指標を表示
            if epoch % 1 == 0:
                print(f"  Epoch: {epoch+1}/{num_epochs}, Total_Loss_Train: {loss_train_fold:.4f}, Total_Loss_val: {loss_val_fold:.4f}, Accuracy_val: {accuracy_val_fold:.4f}, Precision_val: {precision_val_fold:.4f}, Recall_val: {recall_val_fold:.4f}")

            if loss_val_fold < best_val_loss:
                best_val_loss = loss_val_fold
                counter = 0
                
            else:
                counter += 1
                if counter >= patience:
                    print(f"Early Stopping: {patience}エポック連続でtotal_val_loss改善なし。学習を停止します。")
                    break
